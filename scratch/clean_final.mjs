...
conn.on('ready', () => {
  console.log('✅ Финальная очистка и настройка спецзабор.рф...');
  conn.exec(`
    # Конфиг Nginx для спецзабор.рф (xn--80acki1bbmb1c)
...
EOF
  ...
  `, (err, stream) => {
    if (err) throw err;
    stream.on('close', () => {
      console.log('🏁 Всё готово. Проверяйте!');
      conn.end();
