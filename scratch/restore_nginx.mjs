...
conn.on('ready', () => {
  console.log('✅ Очистка и настройка иконки...');
  conn.exec(`
    # Возвращаем рабочий конфиг БЕЗ лишнего SSL (потому что Cloudflare сам справляется)
...
EOF
  ...
  `, (err, stream) => {
    if (err) throw err;
    stream.on('close', () => {
      console.log('🏁 Конфиг очищен, иконка прописана. Проверяйте!');
      conn.end();
